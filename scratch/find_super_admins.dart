...
void main() async {
  ...
  final url = "https://wnxeohqejdiytqkxdcwe.supabase.co/rest/v1/profiles?select=*";
  ...
  try {
    ...
    if (res.statusCode == 200) {
      ...
      print("Admins and Super Admins found:");
      for (var p in profiles) {
        final String role = p['role']?.toString() ?? '';
        if (role.contains('admin')) {
          print("  - ID: ${p['id']} | Name: ${p['full_name']} | Role: ${p['role']} | Username: ${p['username']} | Level: ${p['level']}");
        }
      }
    } else {
      print("Failed to query profiles: ${res.statusCode} - ${res.body}");
    }
  } catch (e) {
    print("Error: $e");
  }
