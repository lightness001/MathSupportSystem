...
class _MathQuizScreenState extends State<MathQuizScreen>
  ...
  final Map<int, List<Map<String, dynamic>>> _questionBank = {
    1: [
      ...
      {
        'q': 'Which is the area of a rectangle?',
        'a': 'L × W',
        'type': 'mcq',
        'topic': 'Geometry',
        'options': ['L + W', 'L × W', '2 × (L+W)'],
      }
    ],
    2: [
  ...
  Future<void> _fetchHomeworkQuestions() async {
    try {
      ...
      if (response is List) {
        for (final hw in response) {
          if (hw is Map) {
            final qList = hw['questions'];
            final String title = hw['title']?.toString() ?? 'Revision';
            if (qList is List) {
  ...
  void _startHomeworkRevision() {
    ...
    setState(() {
      ...
      _questions = List<Map<String, dynamic>>.from(_homeworkQuestions)..shuffle();
      ...
      _errorText = null;
      _progressAnim = Tween<double>(begin: 0, end: 1 / _questions.length)
  ...
  void _loadLevel(int level) {
    final selectedQuestions = List<Map<String, dynamic>>.from(
      _questionBank[level] ?? _questionBank[1]!,
    )..shuffle();
  ...
  void _checkAnswer() {
    ...
    final String type = q['type']?.toString() ?? 'text';
    final String studentAnswer = type == 'mcq'
        ? (_selectedOption ?? '')
    ...
    final bool correct = studentAnswer.trim().toLowerCase() ==
  ...
  Future<void> _finishQuiz() async {
    final String topic = _isHomeworkMode
        ? 'Homework Revision'
        : 'Mixed Practice Level $_currentLevel';
    ...
    final bool isMastered = result.grade == 'A' || result.grade == 'B';
    final bool levelUnlocked = !_isHomeworkMode && isMastered && _currentLevel < 5;
    ...
    await DBHelper().savePerformance({
      ...
      'recommendation': _isHomeworkMode
          ? (isMastered
              ? 'Excellent homework revision work!'
              : 'Keep practicing to master your homework questions.')
          : (levelUnlocked
              ? 'Congrats! Level ${_currentLevel + 1} Unlocked.'
              : result.recommendation),
      'is_synced': 0,
    });

    if (mounted) {
      _showResultDialog(result, levelUnlocked, isMastered);
    }
  }
                    ...
                    Expanded(
                      child: ElevatedButton(
                        ...
                        child: Text(
                          _isHomeworkMode
                              ? 'Practice Again'
                              : (levelUnlocked ? 'Next Level' : (isMastered ? 'Play Again' : 'Retry')),
                ...
                if (isMcq)
                  ...?((q['options'] as List?)
                      ?.map((opt) {
                        ...
                        final bool selected = _selectedOption == optionText;
