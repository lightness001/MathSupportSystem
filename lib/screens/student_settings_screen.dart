...
class StudentSettingsScreen extends StatefulWidget {
  ...
  const StudentSettingsScreen({super.key, required this.studentLevel});
...
class _StudentSettingsScreenState extends State<StudentSettingsScreen> {
  ...
  Future<File> get _settingsFile async {
    ...
    return File('${directory.path}/student_settings_config.json');
  }
  ...
  Future<void> _loadSettingsAndProfile() async {
    try {
      ...
      if (user != null) {
        ...
        _fullName = profile != null ? (profile['full_name'] ?? 'Mwanafunzi') : 'Student';
  ...
  Future<void> _performRealSync() async {
    ...
    try {
      final user = supabase.auth.currentUser;
      if (user != null) {
        // Fetch all student available homework to store locally for offline mode
        final homeworkRes = await supabase.from('homework').select().eq('level', widget.studentLevel);
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.start,
          children: const [
            Text("MathSupport Student Portal", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
            SizedBox(height: 5),
